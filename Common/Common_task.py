...
def create_task(case_ids,task_name,remark,UUID):
    for case_id in case_ids:
        case_data=Lyzd_Interface.objects.filter(id=case_id).values("interface_name_zh","interface_name_en")[0] #接口内得信息：接口名称+接口信息
        # 得到外键数据
        interface_name = Lyzd_Interface.objects.get(id=case_id)
        Lyzd_Task.objects.create(task_name=task_name,uuid=UUID,interface_name=interface_name,remark=remark,status=0)
...
def get_py_data(case_ids,testcasedir,task_name,case_name=None):
    ...
    for case_id in case_ids:
        ...
        interface_data=Lyzd_Interface.objects.filter(id=case_id).values("interface_name_zh","interface_name_en","content","relation","request_way")[0]
        ...
        if case_name:
            # 测试案例表取值
            Interface_Case_data=Lyzd_Interface_Cases.objects.filter(interface_name=interface_name,status=1,case_name=case_name).values("id","case_name","check_key",
                                                                                                                            "case_type","check_type","header_1","header_2",
                                                                                                                            "check_condition","check_value","action_condition")
            # 请求头表去请求头值
            Interface_Header_data = Lyzd_Interface_Header.objects.filter(interface_name=interface_name).values("id","head_key")
            # 入参表取值
            Interface_Case_Param_data = Lyzd_Interface_Case_Param.objects.filter(interface_name=interface_name).values("id","param_key","param_value")

        else:
            Interface_Case_data = Lyzd_Interface_Cases.objects.filter(interface_name=interface_name, status=1).values("id","case_name","check_key",
                                                                                                                  "case_type","check_type","header_1","header_2",
                                                                                                           "check_condition","check_value","action_condition")
            Interface_Header_data = Lyzd_Interface_Header.objects.filter(interface_name=interface_name).values("id","head_key")
            # 入参表取值
