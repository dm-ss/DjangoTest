...
#高靖宇测试序列化否则报错不是json格式
class SysEnvironmentSerializer(serializers.ModelSerializer):
...
class SysInterfaceSerializer(serializers.ModelSerializer):
    ...
    createTime = serializers.DateTimeField(format="%Y-%m-%d %H:%M:%S", required=False, read_only=True)
    class Meta:
        model = Lyzd_Interface

        fields = ('id','interface_name_zh','interface_name_en','content',
                  'LastUpdateTime', 'createTime','requestType','authentication','encryption',
                  'decrypt','delete_flag','create_user','modify_user','requestParameterType','Project_id')
